 ...
 * {@link LightLevelOrderTreeNodeIterator}.
 * 
 * @param <T>
 *            the type of tree node to iterate
 */
public class LightLevelOrderTreeNodeIteratorFactory<T extends TreeNode<T>>
		implements TreeNodeIteratorFactory<T> {

	private int depthLimit;

	public LightLevelOrderTreeNodeIteratorFactory() {
		this.depthLimit = TreeNodeIterator.NO_DEPTH_LIMIT;
	}

	public LightLevelOrderTreeNodeIteratorFactory(final int depthLimit) {
	...
	public TreeNodeIterator<T> create(final T root) {
		if (depthLimit == -1) {
			return new LightLevelOrderTreeNodeIterator<T>(root);
		} else {
			return new LightLevelOrderTreeNodeIterator<T>(root, depthLimit);
		}
	}
