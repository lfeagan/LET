 ...
 * {@link LightPreOrderTreeNodeIterator}.
 * 
 * @param <T>
 *            the type of tree node to iterate
 */
public class LightPreOrderTreeNodeIteratorFactory<T extends TreeNode<T>>
		implements TreeNodeIteratorFactory<T> {

	private int depthLimit;

	public LightPreOrderTreeNodeIteratorFactory() {
		this.depthLimit = TreeNodeIterator.NO_DEPTH_LIMIT;
	}

	public LightPreOrderTreeNodeIteratorFactory(final int depthLimit) {
	...
	public TreeNodeIterator<T> create(final T root) {
		if (depthLimit == TreeNodeIterator.NO_DEPTH_LIMIT) {
			return new LightPreOrderTreeNodeIterator<T>(root);
		} else {
			return new LightPreOrderTreeNodeIterator<T>(root);
		}
	}
